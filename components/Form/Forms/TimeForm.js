...
const timeValue =['8:00','9:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00','21:00','22:00'];
...
// submit
  ...
  return (
      ...
      <div style={{display: 'flex', justifyContent: 'center' }}>
        <div>
          <p className={classes.time} style={{ color: 'white'}}>b</p>
          {timeValue.map((item)=><p className={classes.time}>{item}</p>)}
        </div>
        <div>
          <div style={{display: 'flex', justifyContent: 'space-around', margin: 'auto'}}>
             { weekday.map((item)=> <p className={classes.day}>{item}</p>)}
          </div>
        {times.map((x, xi) => {
          return x.map((y, yi) => {
            return (
              <div style={{ textAlign: 'center', margin: 'auto', borderBottom: '0.5px solid #707070'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                  {
                    Object.entries(y).map(([key, value]) => {
                      return (
                          <>
                          <TimetableButton 
                            value={inTheList('d'+ parseInt(`${key}`)+'t'+parseInt(`${xi}`))}
                            id={'d'+ parseInt(`${key}`)+'t'+parseInt(`${xi}`)} 
                            key={'d'+ parseInt(`${key}`)+'t'+parseInt(`${xi}`)} 
